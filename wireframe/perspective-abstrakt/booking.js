(async()=>{
 ...
 try{
  ...
  const loadCalendar=()=>{if(container.dataset.started==='true')return;if(!window.Calendly){error.textContent='Bitte öffne die Terminbuchung über den Link unter dem Kalender.';return;}container.dataset.started='true';window.Calendly.initInlineWidget({url:calendarUrl.href,parentElement:container,prefill:{name:lead.name,email:lead.email,customAnswers:{a1:lead.phone}}});};
