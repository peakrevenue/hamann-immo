...
export function webhookPayload(lead,event) {
 ...
 return {event,event_id:lead.id+':'+event+':'+revision,lead_id:lead.id,name:lead.name,email:lead.email,phone:lead.phone,country:lead.country,role:lead.role||null,experience:lead.experience||null,income:lead.income||null,qualification:lead.qualification||'pending',stage:lead.stage||'contact_saved',created_at:lead.createdAt,updated_at:lead.updatedAt||lead.createdAt,consented_at:lead.consentedAt,consent_version:lead.consentVersion,experiment:lead.experiment,variant:lead.variant,attribution:utms,...utms};
}
export async function deliverEvent(store,config,key,fetcher=fetch) {
 ...
 if(!config.webhookUrl || config.webhookDisabled)return;
 const rank={contact_created:0,contact_updated:1,quiz_completed:2};
 ...
 const url=new URL(config.webhookUrl);if(url.protocol!=='https:'||url.hostname!=='hooks.zapier.com'||!url.pathname.startsWith('/hooks/catch/'))throw Error('Invalid webhook configuration');
 try {
...
export async function flushOutbox(store,config,fetcher=fetch) {
 if(!config.webhookUrl||config.webhookDisabled)return;
 const jobs=(await store.list('outbox/')).filter(j=>!j.deliveredAt&&j.nextAttemptAt<=Date.now()).slice(0,10);
