...
export async function deliverEvent(store,config,key,fetcher=fetch) {
 ...
 const destination=job.payload.event==='quiz_completed'?config.quizWebhookUrl:config.webhookUrl;
...
export async function flushOutbox(store,config,fetcher=fetch) {
 if((!config.webhookUrl&&!config.quizWebhookUrl)||config.webhookDisabled)return;
 const jobs=(await store.list('outbox/')).filter(j=>!j.deliveredAt&&j.nextAttemptAt<=Date.now()&&(j.payload.event==='quiz_completed'?config.quizWebhookUrl:config.webhookUrl)).slice(0,10);
