...
function contact(){
 ...
 const button=text('button','Absenden');button.className='button';button.type='submit';button.addEventListener('click',validate);form.append(button,notice);
 form.addEventListener('submit',async event=>{event.preventDefault();if(sending)return;validate();if(!form.reportValidity())return;sending=true;button.disabled=true;button.textContent='Wird gespeichert …';error.textContent='';
  try{if(preview)throw Error('In der Admin-Vorschau werden keine Kontaktdaten gespeichert.');await request('contacts',{...Object.fromEntries(new FormData(form)),consent:true,attribution:readAttribution()});location.href=funnelUrl('/quiz/');}catch(e){error.textContent=e.message;}finally{sending=false;button.disabled=false;button.textContent='Absenden';}});root.append(form);
}
function quiz(){
